...
var InformalcommonsWidget = function(options)
{
    ...
    */
    this.attachSearchHandler = function()
    {
		...
		var containerElem = document.getElementById(config.containerId);
		// the button(s)
		var elems = containerElem.getElementsByClassName('informalcommonsWidget-searchButton');
        if(elems.length)
        {
        	for(var i=0; i<elems.length; ++i)
	        {
	            this.attachEvent(elems[i], 'click', this.searchSubmitHandler);
        	}
		}
		// the input element, for "enter" handler
		var inputs = containerElem.getElementsByClassName('informalcommonsWidget-term');
		if(inputs.length)
		{
			// there shouldn't really be more than one of these, but add handler to all just in case
			for(i=0; i<inputs.length; ++i)
			{
				this.attachEvent(inputs[i], 'keypress', this.inputEnterHandler);
			}
		}
    }
    ...
    {
        options = options || {};
		if(!options.hasOwnProperty('encodeParameters'))
			options.encodeParameters = true;
    ...
    /**
     * This method is fired when the search is submitted.
     * @see {@link InformalcommonsWidget#searchBarHTML}
     */
    this.searchSubmitted = function(elem)
    {
		var termInputElem = elem.parentElement.getElementsByClassName('informalcommonsWidget-term')[0];
        var term = termInputElem.value;
        this.search({'qq':term});
    }
    
    this.searchSubmitHandler = function(evt)
    {
        evt = evt || window.event;
        var tgt = evt.target || evt.srcElement;
        _this.searchSubmitted(tgt);
	...
	this.inputEnterHandler = function(evt)
	{
		// debugger;
		evt = evt || window.event;
		if(evt.which == 13)
 ...
 * contains an input of type text with the class "informalcommonsWidget-term" or an
 * input of type button with class informalcommonsWidget-searchButton
 * you may need to change 
 * {@link InformalcommonsWidget#attachSearchHandler} and 
 * {@link InformalcommonsWidget#searchSubmitted} as well.
 */
InformalcommonsWidget.prototype.searchBarHTML = '<div class="informalcommonsWidget-search">'
                            ...
                            + '<input type="button" value="search" class="informalcommonsWidget-searchButton"></input>'
                            + '</div>';
...
InformalcommonsWidget.prototype.searchResultsPaginationHtml = function(data)
{
    ...
    var prevBtn = '<span class="informalcommonsWidget-pagination-button prev'
                + prevExtraClass + '"><img src="http://higherlogicdownload.s3.amazonaws.com/ASTC/bb3be056-bfde-43a2-ac96-4ceb7c97fa01/UploadedImages/arrow_left.png"> Previous</span>';
    ...
    var nextBtn = '<span class="informalcommonsWidget-pagination-button next'
                + nextExtraClass + '">Next <img src="http://higherlogicdownload.s3.amazonaws.com/ASTC/bb3be056-bfde-43a2-ac96-4ceb7c97fa01/UploadedImages/arrow_right.png"></span>';
